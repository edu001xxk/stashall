...
function saveData(key, item) {
    ...
    let data = {
        ip: item.ip,
        ping: Math.round(cleanNum(item.ping)),
        bw: Math.round(cleanNum(item.bandwidth))
    };
    $persistentStore.write(JSON.stringify(data), key);
}

$httpClient.get({ url: url, timeout: 15000 }, function(error, response, data) {
    if (error) { $notification.post("CF优选", "", "网络错误"); $done(); return; }
    try {
        ...
        let msg = `更新成功\n移:${cm?.ip}\n电:${ct?.ip}\n联:${cu?.ip}\nv6:${v6?.ip}`;
        $notification.post("CF全网优选完成 ✅", "", msg);
        console.log(msg);
    } catch (e) {
        $notification.post("CF脚本错误", "", e.message);
    }
    $done();
});
